...
# The venv sees system site-packages, so pip skips any dependency Fedora
# happened to ship at install time. When that RPM later disappears (dnf
# autoremove took python3-typing-extensions) gext dies on import. Inject the
# missing piece into the venv so it no longer depends on the system copy.
